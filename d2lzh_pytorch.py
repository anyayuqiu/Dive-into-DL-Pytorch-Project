...
def train_and_predict_rnn(rnn, get_params, init_rnn_state,
                          num_hiddens, vocab_size, device, corpus_indices,
    ...
    for epoch in range(num_epochs):
        if not is_random_iter:  # 如使⽤用相邻采样，在epoch开始时初始化隐藏状态
            state = init_rnn_state(batch_size, num_hiddens, device)
        l_sum, n, start = 0.0, 0, time.time()
        data_iter = data_iter_fn(corpus_indices, batch_size, num_steps, device)

        for X, Y in data_iter:
            if is_random_iter:  # 如使⽤用随机采样，在每个⼩批量更新前初始化隐藏状态
                state = init_rnn_state(batch_size, num_hiddens, device)
            else:
                # 否则需要使⽤用detach函数从计算图分离隐藏状态, 这是为了
...
# 本类已保存在d2lzh_pytorch包中⽅方便便以后使⽤用
