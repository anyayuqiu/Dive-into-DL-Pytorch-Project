...
d2l.use_svg_display()
d2l.set_figsize()
img = Image.open('../data/img/IMG_1.jpg')
# plt.imshow(img)
# plt.show()
...
#1 翻转和裁剪
# #RandomHorizontalFlip() 一般概率左右翻转
# apply(img, torchvision.transforms.RandomHorizontalFlip())
#
# #RandomVerticalFlip() 一般概率上下翻转
# apply(img, torchvision.transforms.RandomVerticalFlip())

#随机裁剪出一块面积为原面积的区域，且该区域的宽和高之比随机取自0.5 - 2， 缩放到200像素。
shape_aug = torchvision.transforms.RandomResizedCrop(200, scale=(0.1, 1), ratio=(0.5, 2))
#apply(img, shape_aug)

#2 变化颜色
#可以从4个⽅方面改变图像的颜⾊色：亮度（ brightness ）、对⽐比度（ contrast ）、饱和度（ saturation ）和⾊色调（ hue ）
...
def train_with_data_aug(train_augs, test_augs, lr=0.001):
    batch_size, net = 256, d2l.Residual()
